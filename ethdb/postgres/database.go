...
var (
	hasPgStr    = "SELECT exists(select 1 from evm_data WHERE key = $1 LIMIT 1)"
	getPgStr    = "SELECT value FROM evm_data WHERE key = $1"
	putPgStr    = "INSERT INTO evm_data (key, value) VALUES ($1, $2) ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;"
	deletePgStr = "DELETE FROM evm_data WHERE key = $1"
...
func (d *Database) ModifyAncients(f func(ethdb.AncientWriteOp) error) (int64, error) {
	fmt.Println("ModifyAncients nil")
	return 0, nil
}
...
func (d *Database) NewSnapshot() (ethdb.Snapshot, error) {
	fmt.Println("NewSnapshot nil")
	return nil, nil
}

// AncientDatadir returns an error as we don't have a backing chain freezer.
func (d *Database) AncientDatadir() (string, error) {
	fmt.Println("AncientDatadir nil")
	return "", nil
}
